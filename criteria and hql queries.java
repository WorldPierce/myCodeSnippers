@Override
    ...
    //      DetachedCriteria criteriaDetached = DetachedCriteria.forClass(A8spt226.class, "d");
//      criteriaDetached.add(Restrictions.like("fedWireData", caseNumber, MatchMode.ANYWHERE));
//      ProjectionList projList = Projections.projectionList();
//      projList.add(Projections.property("batchNumber"));
//      projList.add(Projections.property("batchSequenceNum"));
//      criteriaDetached.setProjection(projList);
//
//      Criteria criteria = samsSessionFactory.getCurrentSession().createCriteria(A8spt028.class);
//      criteria.add(Subqueries.propertiesIn(new String[]{"batchNumber", "batchSequenceNum"}, criteriaDetached));
//      criteria.setProjection(Projections.property("sequenceAmount"));
//
//      List results = criteria.list();
//      return (BigDecimal) results.get(0);

    StringBuilder hql =  new StringBuilder("select sequenceAmount from A8spt028 t1 where (t1.batchNumber, t1.batchSequenceNum) in (select t2.batchNumber, t2.batchSequenceNum from A8spt226 t2 where t2.fedWireData like :caseNumber)");
        Query query = samsSessionFactory.getCurrentSession().createQuery(hql.toString());
